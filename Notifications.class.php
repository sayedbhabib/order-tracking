...
class ewdotpNotifications {
	...
	public function __construct() {
		...
		add_action( 'ewd_otp_insert_customer_order', 	array( $this, 'user_order_created_email' ) );

		add_action( 'ewd_otp_admin_order_inserted', 	array( $this, 'user_order_created_email' ) );
		add_action( 'ewd_otp_status_updated', 				array( $this, 'user_status_updated_email' ) );

		// Sales rep notifications
		add_action( 'ewd_otp_insert_customer_order', 	array( $this, 'sales_rep_status_updated_email' ) );
		add_action( 'ewd_otp_status_updated', 				array( $this, 'sales_rep_status_updated_email' ) );
	}
	...
	public function admin_customer_note_email( $order ) {
		...
		$email_id = $ewd_otp_controller->settings->get_setting( 'customer-notes-email' );

		if ( ! $email_id ) { return; }

		$email_address = $ewd_otp_controller->settings->get_setting( 'admin-email' ) ? $ewd_otp_controller->settings->get_setting( 'admin-email' ) : get_option( 'admin_email' );
		...
		if ( $email_id < 0 ) {
			...
			$this->send_email( $email_id, $email_address, $order );
		}
	}
	...
	public function admin_customer_order_email( $order ) {
		...
		$email_id = $ewd_otp_controller->settings->get_setting( 'customer-order-email' );
	 ...
	 * @since 3.0.0
	 */
	public function user_order_created_email( $order ) {
		global $ewd_otp_controller;
		...
		foreach ( $statuses as $status ) {
			...
			if ( $status->status == $order->status ) { $email_id = $status->email; }
		}

		if ( empty( $email_id ) ) { return; }

		$email_addresses = explode( ',', $order->email );

		if ( empty( $email_addresses ) ) { return; }
	 ...
	 * @since 3.0.0
	 */
	public function user_status_updated_email( $order ) {
		global $ewd_otp_controller;

		if ( $ewd_otp_controller->settings->get_setting( 'email-frequency' ) != 'change' ) { return; }
	 ...
	 * @since 3.0.0
	 */
	public function sales_rep_status_updated_email( $order ) {
		global $ewd_otp_controller;

		if ( empty( $ewd_otp_controller->settings->get_setting( 'sales-rep-status-notifications' ) ) ) { return; }

		$email_id = $ewd_otp_controller->settings->get_setting( 'sales-rep-status-email' );
		...
		if ( empty( $sales_rep->email ) ) { return; }
		...
		if ( $email_id < 0 ) {
			...
			$this->send_email( $email_id, $sales_rep->email, $order );
		}
	}
	...
	public function send_email( $email_id, $email_address, $order ) {
		...
		foreach ( $email_messages as $email_message ) {
			...
			$message = $this->substitute_email_text( $this->get_email_template( $email_message ), $order );
			$subject = $this->substitute_email_text( $email_message->subject, $order);
	 ...
	 * @since 3.0.0
	 */
	function substitute_email_text( $text, $order ) {
		global $ewd_otp_controller;
		...
		$tracking_url = add_query_arg( $args, $ewd_otp_controller->settings->get_setting( 'tracking-page-url' ) );
		...
		$custom_fields = ewd_otp_decode_infinite_table_setting( $ewd_otp_controller->settings->get_setting( 'custom-fields' ) );
